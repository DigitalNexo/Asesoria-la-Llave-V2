...
import { Plus, Send, FileDown, ArrowLeft, FileKey, Upload } from 'lucide-react';
...
export default function DomiciliacionPage() {
  ...
  const { data: documents = [], isLoading: loadingDocuments } = useQuery({
    queryKey: ['/api/documents/documents', { type: 'BANKING_DOMICILIATION' }],
    queryFn: () => documentsApi.listDocuments({ type: 'BANKING_DOMICILIATION' }),
  });
  ...
  const { data: templates = [] } = useQuery({
    queryKey: ['/api/documents/templates', { type: 'BANKING_DOMICILIATION' }],
    queryFn: () => documentsApi.listTemplates({ type: 'BANKING_DOMICILIATION', isActive: true }),
  });
  ...
  const handleSend = (document: any) => {
    ...
    sendMutation.mutate({
      id: document.id,
      to: client.email,
      subject: 'Documento de Domiciliación Bancaria - Firma requerida',
      message: 'Adjunto encontrará el documento de domiciliación bancaria. Por favor, fírmelo y devuélvalo.',
    });
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Gestión de documentos de domiciliación bancaria</p>
          </div>
        </div>
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          ...
          <DialogContent className="max-w-xl">
            <DialogHeader>
              <DialogTitle>Nuevo Documento de Domiciliación Bancaria</DialogTitle>
              <DialogDescription>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Documentos de Domiciliación Bancaria</CardTitle>
        ...
        <CardContent>
          {loadingDocuments ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No hay documentos. Crea tu primer documento de domiciliación bancaria.
            </div>
          ) : (
            <Table>
              <TableHeader>
