...
async function createDefaultTemplates() {
  console.log('🚀 Creando plantillas predefinidas...\n');
  ...
  for (const template of defaultTemplates) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`❌ Error al crear ${template.nombre}: ${error}`);
      }
    } catch (error) {
      console.log(`❌ Error de conexión al crear ${template.nombre}:`, error);
    }
  }
