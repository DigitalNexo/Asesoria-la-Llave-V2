...
- **Authentication & Authorization:** JWT for secure login, bcrypt for password hashing, and a robust Role-Based Access Control (RBAC) system with granular permissions. All endpoints use `checkPermission` middleware for authorization. Initial administrator creation is automated on first startup with environment variables.
- **Data Management:** Full CRUD operations with validation across modules (Clients, Taxes, Tasks, Manuals).
- **File Management:** Handles file uploads (PDFs, Word, Excel, images, max 10MB) for taxes and manuals with configurable storage providers (LOCAL/FTP/SMB), AES-256-GCM encrypted credentials, automatic migration, and WebSocket progress tracking.
- **Content Creation:** An advanced internal manuals module features a rich text editor (TipTap v3.7.0) with comprehensive formatting tools, professional table controls, drag & drop image support, versioning, automatic table of contents, templates, tags, and PDF export.
- **Notifications:** Nodemailer sends email notifications, and Socket.IO provides real-time notifications and user presence. A multi-account notification system with encrypted SMTP accounts, template editor, dynamic variable insertion, scheduled sending, and full RBAC is production-ready.
...
- **Admin Panel:** Centralized control for user management (including `isActive` status and safe deletion), roles, activity logs, SMTP configuration, user registration, and GitHub configuration for auto-updates.
...
- **Auto-Update System with Backup/Restore:** Complete self-update system that checks GitHub for new versions, creates automatic backups (database, code zip, file uploads) before updates, and provides rollback capability. Real-time WebSocket logging provides live progress feedback. The system includes version management via `package.json`, an enhanced update process (Git pull, NPM install, Prisma migrations, build, health check), and developer tools for release creation.
...
- **Tax Management:** Supports fiscal models (303, 390, 130, 131), various tax periods, and tracks statuses (PENDIENTE, CALCULADO, REALIZADO) with matrix view and mass filing generation.
