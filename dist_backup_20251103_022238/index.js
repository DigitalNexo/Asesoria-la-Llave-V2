...
var StorageFactory = class {
  ...
  static async getActiveProvider() {
    const activeConfig = await prisma6.storage_configs.findFirst({
      where: { isActive: true }
    });
  ...
  static async getProviderById(configId) {
    const config = await prisma6.storage_configs.findUnique({
      where: { id: configId }
    });
    if (!config) {
...
function registerEpicTasksRoutes(app2) {
  app2.get("/api/tasks/:taskId/comments", authenticateToken, async (req, res) => {
    try {
      ...
      const comments = await prisma7.task_comments.findMany({
        where: { taskId },
        include: {
          users: {
            select: {
  ...
  app2.post("/api/tasks/:taskId/comments", authenticateToken, async (req, res) => {
    try {
      ...
      const task = await prisma7.tasks.findUnique({ where: { id: taskId } });
      if (!task) {
        return res.status(404).json({ error: "Tarea no encontrada" });
      }
      const comment = await prisma7.task_comments.create({
        data: {
          id: randomUUID7(),
      ...
      await prisma7.task_activities.create({
        data: {
          id: randomUUID7(),
          taskId,
          userId: req.user.id,
  ...
  app2.patch("/api/tasks/:taskId/comments/:commentId", authenticateToken, async (req, res) => {
    try {
      ...
      const { contenido } = req.body;
      const comment = await prisma7.task_comments.findUnique({
        where: { id: commentId }
      });
      if (!comment) {
      ...
      const updated = await prisma7.task_comments.update({
        where: { id: commentId },
        data: {
          contenido,
          updatedAt: /* @__PURE__ */ new Date()
  ...
  app2.delete("/api/tasks/:taskId/comments/:commentId", authenticateToken, async (req, res) => {
    try {
      ...
      await prisma7.task_comments.delete({
        where: { id: commentId }
      });
      res.status(204).end();
    } catch (error) {
  ...
  app2.get("/api/tasks/:taskId/attachments", authenticateToken, async (req, res) => {
    try {
      const { taskId } = req.params;
      const attachments = await prisma7.task_attachments.findMany({
        where: { taskId },
        include: {
          users: {
            select: {
  ...
  app2.post("/api/tasks/:taskId/attachments", authenticateToken, uploadTaskAttachment.single("file"), async (req, res) => {
    try {
      ...
      const attachment = await prisma7.task_attachments.create({
        data: {
          id: randomUUID7(),
          tasks: { connect: { id: taskId } },
          users: { connect: { id: req.user.id } },
  ...
  app2.delete("/api/tasks/:taskId/attachments/:attachmentId", authenticateToken, async (req, res) => {
    try {
      ...
      const attachment = await prisma7.task_attachments.findUnique({
      ...
      await prisma7.task_attachments.delete({
        where: { id: attachmentId }
      });
      res.status(204).end();
    } catch (error) {
  ...
  app2.get("/api/tasks/:taskId/time-entries", authenticateToken, async (req, res) => {
    try {
      const { taskId } = req.params;
      const entries = await prisma7.task_time_entries.findMany({
        where: { taskId },
        include: {
          users: {
            select: {
  ...
  app2.post("/api/tasks/:taskId/time-entries", authenticateToken, async (req, res) => {
    try {
      ...
      const entry = await prisma7.task_time_entries.create({
        data: {
          id: randomUUID7(),
          tasks: { connect: { id: taskId } },
          users: { connect: { id: req.user.id } },
  ...
  app2.delete("/api/tasks/:taskId/time-entries/:entryId", authenticateToken, async (req, res) => {
    try {
      ...
      const entry = await prisma7.task_time_entries.findUnique({
        where: { id: entryId }
      });
      if (!entry) {
      ...
      await prisma7.task_time_entries.delete({
        where: { id: entryId }
      });
      res.status(204).end();
    } catch (error) {
  ...
  app2.get("/api/tasks/:taskId/activities", authenticateToken, async (req, res) => {
    try {
      const { taskId } = req.params;
      const activities = await prisma7.task_activities.findMany({
        where: { taskId },
        include: {
          users: {
            select: {
  ...
  app2.get("/api/tasks/:taskId/subtasks", authenticateToken, async (req, res) => {
    try {
      const { taskId } = req.params;
      const subtasks = await prisma7.tasks.findMany({
  ...
  app2.patch("/api/tasks/:taskId/move", authenticateToken, async (req, res) => {
    try {
      ...
      const updatedTask = await prisma7.tasks.update({
        where: { id: taskId },
        data: {
  ...
  app2.get("/api/tasks/analytics/overview", authenticateToken, async (req, res) => {
    try {
      const [
        ...
        prisma7.tasks.count({ where: { is_archived: false } }),
        prisma7.tasks.count({ where: { estado: "PENDIENTE", is_archived: false } }),
        prisma7.tasks.count({ where: { estado: "EN_PROGRESO", is_archived: false } }),
        prisma7.tasks.count({ where: { estado: "COMPLETADA", is_archived: false } }),
        prisma7.tasks.count({
          where: {
...
async function registerRoutes(app2, options) {
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!defaultRoleId) {
          const defaultRole = await prisma8.roles.findUnique({
            where: { name: "Gestor" }
          });
          if (defaultRole) {
  ...
  app2.get("/api/users", authenticateToken2, async (req, res) => {
    try {
      const users = await prisma8.users.findMany({
        select: {
          id: true,
          username: true,
          email: true,
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const currentUser = await prisma8.users.findUnique({
        ...
        const targetUser = await prisma8.users.findUnique({
          where: { id }
        });
        if (!targetUser) {
        ...
        await prisma8.users.update({
          where: { id: currentUserId },
          data: { is_owner: false }
        });
        const newOwner = await prisma8.users.update({
          where: { id },
          data: { is_owner: true }
        });
        await prismaStorage.createActivityLog({
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        await prisma8.users.updateMany({
          where: { is_owner: true },
          data: { is_owner: false }
        });
        const newOwner = await prisma8.users.update({
  ...
  app2.delete(
    ...
    async (req, res) => {
      try {
        ...
        const userToDelete = await prisma8.users.findUnique({
          where: { id },
        ...
        const manuals = await prisma8.manuals.count({ where: { autor_id: id } });
        const activityLogs = await prisma8.activity_logs.count({ where: { usuarioId: id } });
        const auditTrails = await prisma8.audit_trail.count({ where: { usuarioId: id } });
        if (manuals > 0) {
  ...
  app2.delete(
    ...
    async (req, res) => {
      try {
        ...
        const clientTaxes = await prisma8.client_tax.findMany({
          where: { clientId: id }
        });
        const assignmentCount = await prisma8.client_tax_assignments.count({
          where: { clientId: id }
        });
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        await prisma8.client_employees.deleteMany({
          where: { clientId: id }
        });
        if (employeeIds && Array.isArray(employeeIds) && employeeIds.length > 0) {
          await prisma8.client_employees.createMany({
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        if (isPrimary) {
          await prisma8.client_employees.updateMany({
            where: { clientId: id },
            data: { is_primary: false }
          });
        }
        await prisma8.client_employees.upsert({
          where: {
            clientId_userId: {
              clientId: id,
              userId
        ...
        if (isPrimary) {
          ...
          const primaryEmployee = await prisma8.client_employees.findFirst({
  ...
  app2.delete(
    ...
    async (req, res) => {
      try {
        ...
        const user = await prismaStorage.getUser(userId);
        const employeeToDelete = await prisma8.client_employees.findUnique({
          where: {
            clientId_userId: {
              clientId: id,
              userId
        ...
        await prisma8.client_employees.delete({
          where: {
            clientId_userId: {
              clientId: id,
              userId
        ...
        if (employeeToDelete?.is_primary) {
          const remainingEmployee = await prisma8.client_employees.findFirst({
            where: { clientId: id }
          });
          if (remainingEmployee) {
            await prisma8.client_employees.update({
              where: {
                clientId_userId: {
                  clientId: id,
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const transporter2 = nodemailer4.createTransport({
          host,
          port: parseInt(port),
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const updatedUsers = await prisma8.users.updateMany({
          where: { username: "CarlosAdmin" },
          data: { is_owner: true }
        });
        const adminUser = await prisma8.users.findFirst({
          where: { username: "CarlosAdmin" },
          select: { username: true, email: true, is_owner: true }
        });
        const roles = await prisma8.roles.findMany({
          select: {
            id: true,
            name: true,
            is_system: true
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        const config = await prisma8.storage_configs.findFirst({
          where: { isActive: true },
          orderBy: { createdAt: "desc" }
        });
        if (!config) {
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        await prisma8.storage_configs.updateMany({
          where: { isActive: true },
          data: { isActive: false }
        });
        const config = await prisma8.storage_configs.create({
          data: {
            id: randomUUID8(),
            type,
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        const configs = await prisma8.system_config.findMany({
          orderBy: { key: "asc" }
        });
        res.json(configs);
      } catch (error) {
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        const config = await prisma8.system_config.findUnique({
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        const existing = await prisma8.system_config.findUnique({
          where: { key: req.params.key }
        });
        if (!existing) {
          return res.status(404).json({ error: "Configuraci\xF3n no encontrada" });
        ...
        const config = await prisma8.system_config.update({
          where: { key: req.params.key },
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        const repoConfig = await prisma8.system_config.findUnique({
          where: { key: "github_repo_url" }
        });
        const branchConfig = await prisma8.system_config.findUnique({
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        if (repoUrl !== void 0) {
          await prisma8.system_config.upsert({
            where: { key: "github_repo_url" },
            create: {
              id: randomUUID8(),
              key: "github_repo_url",
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const existingRole = await prisma8.roles.findUnique({
          where: { name }
        });
        if (existingRole) {
          return res.status(400).json({ error: "Ya existe un rol con ese nombre" });
        }
        const role = await prisma8.roles.create({
          data: {
            id: randomUUID8(),
            name,
    ...
    async (req, res) => {
      try {
        ...
        if (name && name !== existingRole.name) {
          const duplicateRole = await prisma8.roles.findUnique({
            where: { name }
          });
          if (duplicateRole) {
            return res.status(400).json({ error: "Ya existe un rol con ese nombre" });
        ...
        const role = await prisma8.roles.update({
          where: { id },
          data: updateData,
          include: {
            role_permissions: {
  ...
  app2.delete(
    ...
    async (req, res) => {
      try {
        const { id } = req.params;
        const role = await prisma8.roles.findUnique({
          where: { id }
        });
        if (!role) {
          return res.status(404).json({ error: "Rol no encontrado" });
        ...
        const usersWithRole = await prisma8.users.count({
          where: { roleId: id }
        });
        if (usersWithRole > 0) {
          return res.status(409).json({
        ...
        await prisma8.role_permissions.deleteMany({
          where: { roleId: id }
        });
        await prisma8.roles.delete({
          where: { id }
        });
        await prismaStorage.createActivityLog({
          usuarioId: req.user.id,
    ...
    async (req, res) => {
      try {
        ...
        const rolePermissions = await Promise.all(
          permissionIds.map(
            (permissionId) => prisma8.role_permissions.create({
              data: {
                id: randomUUID8(),
                roleId: id,
                permissionId
        ...
        const updatedRole = await prisma8.roles.findUnique({
          where: { id },
          include: {
            role_permissions: {
              include: {
  ...
  app2.get("/api/tax-requirements", authenticateToken2, async (req, res) => {
    try {
      const requirements = await prisma8.client_tax_requirements.findMany({
        include: { clients: true }
      });
  ...
  app2.post("/api/tax-requirements", authenticateToken2, checkPermission2("taxes:create"), async (req, res) => {
    try {
      ...
      const requirement = await prisma8.client_tax_requirements.create({
        data: {
          id: randomUUID8(),
          clientId,
  ...
  app2.patch("/api/tax-requirements/:id/toggle", authenticateToken2, checkPermission2("taxes:update"), async (req, res) => {
    try {
      const { id } = req.params;
      const current = await prisma8.client_tax_requirements.findUnique({ where: { id } });
      if (!current) {
        return res.status(404).json({ error: "Requisito no encontrado" });
      }
      const updated = await prisma8.client_tax_requirements.update({
        where: { id },
  ...
  app2.get("/api/fiscal-periods", authenticateToken2, async (req, res) => {
    try {
      const periods = await prisma8.fiscal_periods.findMany({
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const list = await prisma8.tax_calendar.findMany({ where, orderBy: [{ endDate: "asc" }] });
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const rows = await prisma8.tax_calendar.findMany({ where: { year: y }, orderBy: [{ startDate: "asc" }] });
        const toICSDate = (d) => {
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const totalFilings = await prisma8.client_tax_filings.count();
        const filingsWithPeriods = await prisma8.client_tax_filings.count({
          where: { fiscal_periods: { isNot: null } }
        });
        const totalPeriods = await prisma8.fiscal_periods.count();
        const periodsThisYear = await prisma8.fiscal_periods.count({ where: { year } });
        const periods = await prisma8.fiscal_periods.findMany({
          select: { year: true },
          distinct: ["year"],
          orderBy: { year: "desc" }
        });
        const filingsThisYear = await prisma8.client_tax_filings.count({
          where: { fiscal_periods: { year } }
        });
        const sampleFilings = await prisma8.client_tax_filings.findMany({
          take: 3,
          include: {
            fiscal_periods: true,
            clients: { select: { razonSocial: true } }
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        const allFilings = await prisma8.client_tax_filings.findMany({
          include: {
            fiscal_periods: true
          }
        });
        const assignments = await prisma8.client_tax_assignments.findMany({
        ...
        if (orphanIds.length > 0) {
          await prisma8.client_tax_filings.deleteMany({
  ...
  io2.on("connection", (socket) => {
    ...
    heartbeatInterval = setInterval(async () => {
      try {
        await prisma8.sessions.updateMany({
...
var taskRemindersJob = cron.createTask("0 9 * * *", async () => {
  ...
  try {
    ...
    const upcomingTasks = await prisma9.tasks.findMany({
      where: {
...
var taxRemindersJob = cron.createTask("0 8 * * *", async () => {
  ...
  try {
    ...
    const clientes = await prisma9.clients.findMany({
      include: {
        clientTaxes: {
          include: {
            period: {
...
var taxCalendarRefreshJob = cron.createTask("0 */6 * * *", async () => {
  ...
  try {
    const entries = await prisma9.tax_calendar.findMany();
    let updated = 0;
    for (const entry of entries) {
      ...
      if (entry.status !== derived.status || entry.days_to_start !== derived.daysToStart || entry.days_to_end !== derived.daysToEnd) {
        await prisma9.tax_calendar.update({
          where: { id: entry.id },
          data: {
            status: derived.status,
            days_to_start: derived.daysToStart,
...
var fiscalPeriodsStatusJob = cron.createTask("0 */6 * * *", async () => {
  ...
  try {
    ...
    const periods = await prisma9.fiscal_periods.findMany({
      select: {
        id: true,
        starts_at: true,
        ends_at: true,
    ...
    for (const period of periods) {
      ...
      if (newStatus && newStatus !== period.status) {
        await prisma9.fiscal_periods.update({
          where: { id: period.id },
...
var cleanupSessionsJob = cron.createTask("0 * * * *", async () => {
  ...
  try {
    const prisma10 = new PrismaClient9();
    const sevenDaysAgo = /* @__PURE__ */ new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const closedSessionsResult = await prisma10.sessions.deleteMany({
      where: {
        ended_at: {
          not: null,
          lt: sevenDaysAgo
    ...
    const inactiveSessionsResult = await prisma10.sessions.updateMany({
      where: {
        ended_at: null,
